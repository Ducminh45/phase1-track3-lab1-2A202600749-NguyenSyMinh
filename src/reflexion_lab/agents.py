...
class BaseAgent:
    ...
    def run(self, example: QAExample) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            answer, tokens_a, lat_a = actor_answer(example, attempt_id, self.agent_type, reflection_memory)
            judge, tokens_j, lat_j = evaluator(example, answer)
            
            token_estimate = tokens_a + tokens_j
            latency_ms = lat_a + lat_j
            ...
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts:
                reflection, tokens_r, lat_r = reflector(example, attempt_id, judge)
                token_estimate += tokens_r
                latency_ms += lat_r
