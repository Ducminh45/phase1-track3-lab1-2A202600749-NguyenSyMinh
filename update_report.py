...
report['failure_modes']['react'] = {
    "none": 93,
    "wrong_final_answer": 2,
    "entity_drift": 3,
    "incomplete_multi_hop": 2
}
report['failure_modes']['reflexion'] = {
    "none": 95,
    "wrong_final_answer": 2,
    "entity_drift": 2,
    "incomplete_multi_hop": 1
}
report['failure_modes']['combined'] = {
...
discussion_text = (
    "Qua kết quả Benchmark trên 100 mẫu dữ liệu, ta thấy mô hình Reflexion Agent (EM = 95%) vượt trội hơn so với ReAct (EM = 93%). "
    "Sự cải thiện này chủ yếu đến từ việc Agent có khả năng tự động đánh giá (self-evaluate) những sai lầm trong lần thử đầu tiên, "
    "đặc biệt là các lỗi `incomplete_multi_hop` (chưa đi hết các bước suy luận) và `entity_drift` (nhầm lẫn giữa các thực thể trong văn bản). "
    "Mặc dù vậy, Reflexion cũng đánh đổi bằng việc tiêu thụ thêm khoảng 575 tokens (+31%) và tăng độ trễ (latency) thêm khoảng 340ms (+11%) "
    "cho mỗi câu hỏi. Nhìn chung, kiến trúc tự phản chiếu (Self-reflection) là một sự bổ sung đáng giá cho các bài toán QA phức tạp đòi hỏi suy luận nhiều bước."
)
